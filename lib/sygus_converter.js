...
function getInputOutputType(intendedExamples, fxn){
  ...
  var types = new Array();
  var outputType;
  var conversionDict = {"number": "Int", "string": "String", "boolean": "Bool"};
  var regExp = /\(([^)]+)\)/;

  for (i=0; i < intendedExamples.length; i+=2) {
    if(intendedExamples[i].includes(fxn.name)){
    var params = (regExp.exec(intendedExamples[i])[1]).split(",");
    for (j=0; j < params.length; j++) {
      paramTypes[(fxn.params)[j].name] = conversionDict[typeof eval(params[j])];
    }

    outputType = (typeof intendedExamples[i+1]).toString();
    console.log("output type of "+ fxn.name+ "(): " + outputType)

    types[0] = paramTypes;
    console.log(JSON.stringify(paramTypes));
    types[1] = conversionDict[outputType];
...
function genGrammar(params, intendedExamples, inTypes, outType) {
  ...
  for (i=0; i < intendedExamples.length; i+=1) {
    if (typeof intendedExamples[i] == "string"){
    ...
    for (j=intendedExamples[i].indexOf("(")+1; j < intendedExamples[i].length; j++) {
      if (intendedExamples[i][j]!="\"") {
         if (!isNaN(parseInt(intendedExamples[i][j]))) {
         if ((Object.keys(consts)).includes("number")){
           consts["number"].add(String(parseInt(intendedExamples[i][j])));
         } else{
           consts["number"] = new Set(String(parseInt(intendedExamples[i][j])));

         }

       } else {
         var type = getType(intendedExamples[i][j]);
         if ((Object.keys(consts)).includes(type)){
           consts[type].add(JSON.stringify(intendedExamples[i][j]));
         } else {
           consts[type] = new Set();
           consts[type].add(JSON.stringify(intendedExamples[i][j]))
         }
...
(ntString String (`+
  ((params.map(c => c.name)).filter(param => inTypes[param] == "String")).join(" ") + " " +
  (Array.from(consts["string"]).join(" ")) +
  `(str.++ ntString ntString)
  (str.replace ntString ntString ntString)
  (str.at ntString ntInt)
  (str.substr ntString ntInt ntInt)))
(ntInt Int (`+ ((params.map(c => c.name)).filter(param => inTypes[param] == "Int")).join(" ") + " " +
(Array.from(consts["number"]).join(" ")) +`
  (+ ntInt ntInt)
  (- ntInt ntInt)
  (str.len ntString)
  (str.indexof ntString ntString ntInt)))
(ntBool Bool (`+ ((params.map(c => c.name)).filter(param => inTypes[param] == "Bool")).join(" ") + " " +
(Array.from(consts["boolean"]).join(" ")) +
