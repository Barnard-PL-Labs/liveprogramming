...
function genGrammar(params, intendedExamples, inTypes, outType) {
  ...
  for (i=0; i < intendedExamples.length; i+=1) {
    ...
    for (j=intendedExamples[i].call.indexOf("(")+1; j < intendedExamples[i].call.length; j++) {
      if (intendedExamples[i].call[j]!="\"") {

        if(j < intendedExamples[i].call.length && j>0 ){

          var isLetterRight = isLetter(intendedExamples[i].call[j+1]);
          var isLetterLeft = isLetter(intendedExamples[i].call[j-1]);
        }
         if (!isNaN(parseInt(intendedExamples[i].call[j])) && !(isLetterRight || isLetterLeft)) {
