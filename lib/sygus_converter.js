  ...
  for (i=0; i < intendedExamples.length; i+=1) {
    ...
    for (j=intendedExamples[i].indexOf("(")+1; j < intendedExamples[i].length; j++) {
      if (intendedExamples[i][j]!="\"") {
        ...
        if(j < intendedExamples[i].length && j>0 ){

          var isLetterRight = isLetter(intendedExamples[i][j+1]);
          var isLetterLeft = isLetter(intendedExamples[i][j-1]);
        }
         if (!isNaN(parseInt(intendedExamples[i][j])) && !(isLetterRight || isLetterLeft)) {
